//============================================================================
// Name        : Puzzle-Solution-version2.cpp
// Author      : YuTing-Lai
// Version     :
...
void Dual(){
	...
	//if the blank is not at row(s-1) col(s-1)
	if(puzzle[s-1][s-1]==number){
		...
		//put the blank to the right-down-most of the puzzle
		if(rowB == s-1){
			swap_number('d');
			printMoveSteps(N);
			print_puzzle();
...
void PuzzleStack(int width){
	...
	if(puzzle[1+s-width][1+s-width] != s*s){
		//find the blank first
		for(pos=0; pos<N; pos++){
			if(puzzle[pos/s +1][pos%s +1] == N)
				break;
		...
		int finished_pos=1;
		while(puzzle[s-width+1][s-width+finished_pos] == number){
			finished_pos++;
			number++;
			if(number == (s*s-(width-1)*s) ){
				break;
			}
		}

		if(number != s*s-(width-1)*s){
			int RD = s-width+1 - rowB;
			int CD = s-width+finished_pos - colB;
			//move the blank to the left-up-most side of the puzzle
			if(finished_pos > colB){
				for(int i=0; i<CD; i++){
					swap_number('r');
				}
			}
	...
	if( (puzzle[s-width+2][s]!=N) && puzzle[s-width+1][s]==number){
		number = s*s - (width-2)*s - width+1;
	}
	else{
		if(puzzle[s-width+2][s] != N){
			//Find the blank's position
			for(pos=0; pos<N; pos++){
				if(puzzle[pos/s +1][pos%s +1] == N)
					break;
	...
	if(puzzle[s-width+2][s-width+1] != N){
		//find the blank
		for(pos=0; pos<N; pos++){
			if(puzzle[pos/s +1][pos%s +1] == N)
				break;
		...
		int finished_pos=1;
		while(puzzle[s-width+finished_pos+1][s-width+1] == number){
			finished_pos++;
			number+=s;
			if(number == s*s-width+1){
				break;
			}
		}

		if(number != s*s-width+1){
			int CD = s-width+1 - colB;
			int RD = s-width+1+finished_pos - rowB;
			//move the blank to the right position
			for(int i=0; i>CD; i--){
				swap_number('l');
			}
			if(RD>0){
	...
	if( (puzzle[s][s-width+1]==number) && (width==3) && (puzzle[s][s-width+2]==N)){
		Dual();
	}
	else if( (puzzle[s][s-width+2]!=N) && (puzzle[s][s-width+1]==number) ){
		if(width==3){
			Dual();
		}
		else{
